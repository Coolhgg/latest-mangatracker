...
describe('Security and Validation Tests', () => {
  ...
  describe('UUID Validation', () => {
    ...
    it('should reject invalid UUIDs', () => {
      ...
      expect(() => validateUUID('550e8400-e29b-41d4-a716-44665544000g')).toThrow()
    })
  ...
  describe('Rate Limiting', () => {
    ...
    it('should reset after window expires', () => {
      ...
      // Use up all requests
      for (let i = 0; i < 5; i++) {
        checkRateLimit(key, 5, 100) // Short window
      }
    ...
    it('should use stricter limits for auth endpoints', () => {
      const ip = 'test-ip'
      // Auth rate limit is 5 per minute
      for (let i = 0; i < 5; i++) {
  ...
  describe('SQL Injection Prevention', () => {
    // Note: Prisma parameterizes queries, but these tests ensure input is still clean
    it('should handle SQL-like characters safely', () => {
      const input = "'; DROP TABLE users; --"
      const safe = sanitizeInput(input)
      // The string should still be preserved but not execute as SQL
